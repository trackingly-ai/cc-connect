...
func (c *Config) validate() error {
	...
	for i, proj := range c.Projects {
		...
		allowsHeadless := strings.EqualFold(strings.TrimSpace(proj.Role), "reviewer")
		if len(proj.Platforms) == 0 && c.Echo.ServerURL == "" && !allowsHeadless {
			return fmt.Errorf("config: %s needs at least one [[projects.platforms]] or role = \"reviewer\"", prefix)
		}
