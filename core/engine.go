...
func buildReviewPrompt(originProject, reviewerProject, summary string) string {
	return fmt.Sprintf(
		"Please review the following review packet.\n\nOriginal agent: %s\nReviewer: %s\n\nReview packet:\n%s\n\nIf your available native skills include a review skill whose description matches this task, prefer it before answering. In this repository the intended examples are `comprehensive-review`, `code-review`, and `security-review`: prefer `comprehensive-review` for broad or risky changes, `code-review` for a normal pass, and add `security-review` when the packet suggests auth, permissions, secrets, shell execution, file access, or external integrations. If no matching skill is available, follow the instructions below directly.\n\nUse the packet to determine whether you should inspect working tree changes, the latest commit, or summary-only context. Report only findings that matter. Prioritize bugs, regressions, risky assumptions, unclear implementation details, and missing validation or tests. Order findings from highest severity to lowest severity. Do not include praise, strengths, or 'What Works Well' style sections. If no major issue is found, say that explicitly and keep the response brief.",
		originProject,
		reviewerProject,
...
func buildRevisionPrompt(originProject, reviewerProject, originSummary, reviewSummary string) string {
	return fmt.Sprintf(
		"Please revise your previous work based on the following review feedback.\n\nYour previous summary:\n%s\n\nReviewer feedback from %s:\n%s\n\nIf your available native skills include a review-feedback triage skill whose description matches this task, prefer it before changing the implementation. In this repository the intended example is `review-feedback-triage`. If no matching skill is available, do that triage yourself.\n\nUpdate the work accordingly and provide an updated final summary.",
