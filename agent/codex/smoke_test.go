...
func TestAgentSessionSmoke_UsesManagedWorkspaceAndExtraDirs(t *testing.T) {
	...
	if !containsArgPair(args, "--cd", managedDir) {
		t.Fatalf("args missing --cd %q: %#v", managedDir, args)
	}
	if got := strings.TrimSpace(readTextFile(t, cwdFile)); canonicalPath(got) != canonicalPath(managedDir) {
...
func waitForResult(t *testing.T, events <-chan core.Event) {
	t.Helper()
