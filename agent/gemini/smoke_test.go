...
func TestAgentSessionSmoke_PassesReasoningOverrideToGeminiCLI(t *testing.T) {
	baseDir := t.TempDir()
	repoDir := filepath.Join(baseDir, "repo")
	binDir := filepath.Join(baseDir, "bin")
	if err := os.MkdirAll(repoDir, 0o755); err != nil {
	...
	script := "#!/bin/sh\n" +
		...
		"printf '%s\\n' '{\"type\":\"init\",\"session_id\":\"gemini-effort\",\"model\":\"gemini-2.5-pro\"}'\n" +
		"printf '%s\\n' '{\"type\":\"result\",\"status\":\"success\"}'\n"
	scriptPath := filepath.Join(binDir, "gemini")
	if err := os.WriteFile(scriptPath, []byte(script), 0o755); err != nil {
		t.Fatalf("write fake gemini: %v", err)
	...
	agentRaw, err := New(map[string]any{"work_dir": repoDir, "reasoning_level": "high"})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	agent := agentRaw.(*Agent)
	...
	if !strings.Contains(string(data), "\"thinkingBudget\": 4096") {
		t.Fatalf("settings file = %s, want thinkingBudget 4096", string(data))
	}
}
